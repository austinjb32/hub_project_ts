...
const isAuthenticated = () => (next) => async (root, args, context, info) => {
    ...
    const decodedToken = jsonwebtoken_1.default.verify(context.accessToken, 'somesupersecretsecret');
    const foundUser = await user_1.default.findById(decodedToken.userId);
    if (!foundUser) {
        throw new Error('No User Found');
    }
    const formattedUser = { ...foundUser, _id: foundUser?._id.toString(), createdAt: foundUser?.createdAt.toISOString(),
